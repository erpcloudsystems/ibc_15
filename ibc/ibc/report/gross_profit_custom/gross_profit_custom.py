...
EXCLUDED_ITEM_GROUPS = ["Services1", "2-Service"]
EXCLUDED_BRANDS = ["Service"]
...
def execute(filters=None):
	...
	group_wise_columns = frappe._dict({
		"invoice": ["parent", "customer", "customer_group", "posting_date","item_code", "item_name", "brand", \
			"warehouse", "qty", "base_rate", "buying_rate","actual_val","actual_amount", "base_amount",
			"buying_amount", "actual_profit", "actual_profit_percent"],
...
def get_columns(group_wise_columns, filters):
	...
	column_map = frappe._dict({
		...
		"actual_val": _("Actual Value") + ":Currency:100",
		"actual_amount": _("Actual Amount") + ":Currency:100",
		"base_amount": _("Selling Amount") + ":Currency/currency:100",
		"buying_amount": _("Buying Amount") + ":Currency/currency:100",
		"actual_profit": _("Actual Profit") + ":Currency/currency:100",
		"actual_profit_percent": _("Actual Profit %") + ":Percent:100",
...
class GrossProfitGenerator(object):
			...
			if self.filters.get("group_by") != "Invoice":
				...
				for i, row in enumerate(self.grouped[key]):
					if row.parent in self.returned_invoices \
						...
						row.buying_amount = flt(row.qty * row.actual_val, self.currency_precision)
	...
	def load_invoice_items(self):
		...
		excluded_brands = ", ".join(frappe.db.escape(d) for d in EXCLUDED_BRANDS)
		conditions += " and ifnull(`tabSales Invoice Item`.item_group, '') not in ({0})".format(excluded_item_groups)
		conditions += " and ifnull(`tabSales Invoice Item`.brand, '') not in ({0})".format(excluded_brands)
		...
		self.si_list = frappe.db.sql("""
			select
				...
				(select `tabSales Order Item`.name from `tabSales Order Item` where `tabSales Order Item`.name = `tabSales Invoice Item`.so_detail ) as so_dd,

				IF(`tabSales Invoice Item`.stock_qty>0, (select distinct a.incoming_rate from `tabDelivery Note Item` a where a.so_detail = so_dd  and a.incoming_rate!=0 limit 1), `tabSales Invoice Item`.incoming_rate) as actual_val,
				(IF(`tabSales Invoice Item`.stock_qty>0, (select distinct a.incoming_rate from `tabDelivery Note Item` a where a.so_detail = so_dd  and a.incoming_rate!=0 limit 1), `tabSales Invoice Item`.incoming_rate) * `tabSales Invoice Item`.stock_qty) as actual_amount,
				((`tabSales Invoice Item`.base_net_rate*`tabSales Invoice Item`.stock_qty)-(IF(`tabSales Invoice Item`.stock_qty>0, (select distinct a.incoming_rate from `tabDelivery Note Item` a where a.so_detail = so_dd  and a.incoming_rate!=0 limit 1), `tabSales Invoice Item`.incoming_rate) * `tabSales Invoice Item`.stock_qty)) as actual_profit,
				(((`tabSales Invoice Item`.base_net_rate*`tabSales Invoice Item`.stock_qty)-(IF(`tabSales Invoice Item`.stock_qty>0, (select distinct a.incoming_rate from `tabDelivery Note Item` a where a.so_detail = so_dd  and a.incoming_rate!=0 limit 1), `tabSales Invoice Item`.incoming_rate) * `tabSales Invoice Item`.stock_qty))/(IF(`tabSales Invoice Item`.stock_qty>0, (select distinct a.incoming_rate from `tabDelivery Note Item` a where a.so_detail = so_dd  and a.incoming_rate!=0 limit 1), `tabSales Invoice Item`.incoming_rate) * `tabSales Invoice Item`.stock_qty)*100) as actual_profit_percent
				{sales_person_cols}
			from
