...
class Analytics(object):
	...
	def get_sales_transactions_based_on_order_type(self):
		...
		self.entries = frappe.db.sql(""" select s.order_type as entity, s.{value_field} as value_field, s.{date_field}
			from `tab{doctype}` s where s.docstatus = 1 and s.company = %s and s.{date_field} between %s and %s and s.sales_person = %s
	...
	def get_sales_transactions_based_on_items(self):
		...
		if self.filters.brand :
			...
			and s.{date_field} between %s and %s and i.brand = %s
			""".format(date_field=self.date_field, value_field=value_field, doctype=self.filters.doc_type, sales_person=self.filters.sales_person),
			(self.filters.company, self.filters.from_date, self.filters.to_date, self.filters.brand), as_dict=1)
