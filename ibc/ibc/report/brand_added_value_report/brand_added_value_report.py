	...
	item_results = frappe.db.sql(f"""
			select
				...
				ifnull((select sum(amount)
					...
					where `tabPurchase Invoice Item`.brand = `tabBrand`.name
	...
	for item_dict in item_results:
		...
		for warehouse in warehouses:
			...
			if sales_value != 0 and cogs != 0:
				...
				# data['sales_value'] = round(sales_value,2)
				data['added_value'] = round(added_value,2)
				data['cogs/sales_value'] = 0
				data['added_value/sales_value'] = 0
				data['added_value/cogs'] = 0
